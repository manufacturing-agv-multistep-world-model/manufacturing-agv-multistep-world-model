...
if ($Device -eq "cuda") {
  ...
  Write-Host "CUDA detected. V14 terminal-head training will use GPU mixed precision."
} else {
  if ($RequireGpu) {
    throw "GPU training was required, but CUDA is unavailable."
  }
  Write-Warning "CUDA is unavailable. V14 training will run on CPU."
}
...
if (-not (Test-Path -LiteralPath $Cache)) {
  throw "Missing frozen trajectory cache: ${Cache}"
}
...
foreach ($Seed in @(42, 43, 44)) {
  ...
  $OutputDir = Join-Path $ProjectRoot "world_model_runs\pi_gwm_multistep_v14_dual_timescale_v1_seed${Seed}"
  & $Python $Trainer `
    --model-variant v14 `
    ...
    @DeviceArgs
  if ($LASTEXITCODE -ne 0) {
    throw "V14 dual-timescale training failed for seed ${Seed}."
  }
}

Write-Host "V14 dual-timescale training finished for seeds 42, 43, and 44."
