...
& $Python (Join-Path $ProjectRoot "run_multistep_decision_attribution.py") `
    ...
    --max-steps 2000 `
    --output-dir $Output
if ($LASTEXITCODE -ne 0) { throw "N1 energy-neutral 1 h development run failed." }

& $Python (Join-Path $ProjectRoot "analyze_multistep_decision_attribution.py") --result-dir $Output
if ($LASTEXITCODE -ne 0) { throw "N1 energy-neutral 1 h development audit failed." }
Write-Host "N1 energy-neutral 1 h development finished: $Output"
