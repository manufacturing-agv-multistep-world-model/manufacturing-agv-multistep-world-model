...
& $Python (Join-Path $ProjectRoot "run_multistep_decision_attribution.py") `
    ...
    --max-steps 800 `
    --output-dir $Output
if ($LASTEXITCODE -ne 0) { throw "N1 arrival-v4 smoke run failed." }

& $Python (Join-Path $ProjectRoot "analyze_multistep_decision_attribution.py") --result-dir $Output
if ($LASTEXITCODE -ne 0) { throw "N1 arrival-v4 smoke audit failed." }

Write-Host "N1 arrival-v4 baseline smoke run finished: $Output"
